function init_objective_params()
    ...
    @NLconstraint(IOP, [s = 1:NoS], 2*Q[1]*x[s][1]+Q[2]-0.5*λ[s, 1]-
    (a[s]-1)*λ[s, 2]+λ[s, 3]+λ[s, 4]-λ[s, 5] <= slack1[1, s])
    @NLconstraint(IOP, [s = 1:NoS], -(2*Q[1]*x[s][1]+Q[2]-0.5*λ[s, 1]-
    (a[s]-1)*λ[s, 2]+λ[s, 3]+λ[s, 4]-λ[s, 5]) <= slack1[1, s])

    @NLconstraint(IOP, [s = 1:NoS], 2*Q[3]*x[s][2]+Q[4]-(u[s]-T3-170+
    ...
    @NLconstraint(IOP, [s = 1:NoS], (x[s][1]/2 + 553 - T3)*λ[s, 1] 
    <= slack2[1, s])
    @NLconstraint(IOP, [s = 1:NoS], -((x[s][1]/2 + 553 - T3)*λ[s, 1]) 
    <= slack2[1, s])

    @NLconstraint(IOP, [s = 1:NoS], (-λ[s, 2]*10 - λ[s, 2]*x[s][1] + 
    ...
    @NLconstraint(IOP, [s = 1:NoS], (2*T3 - 786 - x[s][1] + 
    (u[s] - 393)*x[s][2])*λ[s, 3] <= slack2[3, s])
    @NLconstraint(IOP, [s = 1:NoS], -((2*T3 - 786 - x[s][1] + 
    (u[s] - 393)*x[s][2])*λ[s, 3]) <= slack2[3, s])

    @NLconstraint(IOP, [s = 1:NoS], ((2*T3 - 1026 - x[s][1] + 
...
function init_constraint_params()
    ...
    @objective(Init, Max, sum(slack)+1e-5*sum(t))
    optimize!(Init)
    return value.(p)
end